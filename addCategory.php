...
$sql = "INSERT INTO registered_participants VALUES ('','$category','$user','$paid')";
...
if ($limit['current_participants'] < $limit['quota']) {
    $sql_check = "SELECT * FROM registered_participants WHERE participant_ic='$user'";
    $result_check = mysqli_query($con, $sql_check);
    $row_check = mysqli_fetch_array($result_check);
    if (isset($row_check)) {
        $sql_update = "UPDATE registered_participants SET paid=$paid WHERE participant_ic='$user'";
        ...
        if ($result) {
            ...
            if (isset($succes_add)) {
                header("location: index.html"); //forward to payment.html
            } else {
